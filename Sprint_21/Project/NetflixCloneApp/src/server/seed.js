...
const createQuery = (name) => `CREATE TABLE ${name} (
	id varchar(255),
  title varchar(255),
...
db.serialize(() => {
  db.run(createQuery('films'));
  db.run(createQuery('series'));

  films.forEach((episodes) => {
    let sql = `INSERT INTO films VALUES ("${episodes.id}", "${episodes.title}", "${episodes.description}", "${episodes.genre}", "${episodes.maturity}", "${episodes.slug}")`;
  ...
  series.forEach((episodes) => {
    let sql = `INSERT INTO series VALUES ("${episodes.id}", "${episodes.title}", "${episodes.description}", "${episodes.genre}", "${episodes.maturity}", "${episodes.slug}")`;
    db.each(sql, (err, data) => {
      if (err) {
        console.log(err);
      }
